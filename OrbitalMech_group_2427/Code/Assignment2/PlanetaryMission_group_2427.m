...
legend('', 'Orbit', 'Eccentricity vector', 'Pericenter');
title('Nominal orbit');
...
% Pericenter anomaly
kep1(:,5) = rad2deg(unwrap(deg2rad(kep1(:,5))));
figure
plot(t_span, (kep1(:,5)), 'b-');
grid on
xlim([0 t_span(end)])
ylim([min(kep1(:, 5)) max(kep1(:, 5))])
xlabel('Time [s]')
ylabel('ω [deg]')
...
% Data import Navstar-6
data = importdata("NAVSTAR_6M_30MIN.txt");
kep2 = [data.data(:,10), data.data(:,1), data.data(:,3), data.data(:,4), data.data(:,5), data.data(:,9)];

% Setting the time interval for the propagation
t_span = 0:(30*60):(183*24*60*60);

% Plot
% Semi-major axis
figure
plot(t_span, kep2(:, 1), 'b-')
grid on
xlim([0 t_span(end)])
ylim([min(kep2(:, 1)) max(kep2(:, 1))])
xlabel('Time [s]')
ylabel('a [km]')
title("Evolution of semi-major axis")
legend('a_e_p_h_e_m')
...
% Eccentricity
figure
plot(t_span, kep2(:,2), 'b-')
grid on
xlim([0 t_span(end)])
ylim([min(kep2(:, 2)) max(kep2(:, 2))])
xlabel('Time [s]')
ylabel('e [-]')
title("Evolution of eccentricity")
legend('e_e_p_h_e_m')
...
% Inclination
figure
plot(t_span, (kep2(:,3)), 'b-')
grid on
xlim([0 t_span(end)])
ylim([min(kep2(:, 3)) max(kep2(:, 3))])
xlabel('Time [s]')
ylabel('i [deg]')
title("Evolution of inclination")
legend('i_e_p_h_e_m')
...
% RAAN
figure
plot(t_span, kep2(:,4),'b-')
grid on
xlim([0 t_span(end)])
ylim([min(kep2(:,4)) max(kep2(:,4))])
xlabel('Time [s]')
ylabel('Ω [deg]')
title("Evolution of RAAN")
legend('Ω_e_p_h_e_m')
...
% Pericenter anomaly
figure
plot(t_span, (kep2(:,5)), 'b-');
grid on
xlim([0 t_span(end)])
ylim([min(kep2(:, 5)) max((kep2(:, 5)))])
xlabel('Time [s]')
ylabel('ω [deg]')
title("Evolution of pericenter anomaly")
legend('ω_e_p_h_e_m')
